...
*   **💾 状态持久化与历史记录**：自动保存分析进度，刷新页面可恢复；支持查看和恢复历史分析记录（最多保存 50 条）。
...
*   **状态持久化**：使用 localStorage 自动保存分析状态，支持页面刷新后恢复进度（数据有效期 30 分钟）。
*   **历史记录**：自动保存完成的分析到历史记录，支持查看、恢复和删除（最多保存 50 条）。

---

## 📜 历史记录功能

系统会自动保存你的分析历史，方便随时查看过往分析结果。
...
|------|------|
| **自动保存** | 分析完成后自动保存到历史记录 |
| **状态恢复** | 刷新页面后可恢复未完成的分析进度 |
| **历史查看** | 点击导航栏「历史」按钮查看所有记录 |
| **恢复分析** | 从历史记录恢复之前的分析结果 |
| **单条删除** | 删除不需要的历史记录 |
| **清空历史** | 一键清空所有历史记录 |
